...
# MAGIC %md
# MAGIC # Silver: `dwh.silver.dim_account`
# MAGIC
# MAGIC Databricks replacement for the Talend job `Load_DimAccount`.
# MAGIC
# MAGIC Talend job (`IDX_INTERNSHIP/process/Load_DimAccount_0.1.item`):
# MAGIC
# MAGIC ```
# MAGIC tMSSqlInput (Sample_DB, table "account")
# MAGIC     SELECT dbo.account.account_id, dbo.account.customer_id, dbo.account.account_type,
# MAGIC            dbo.account.balance, dbo.account.date_opened, dbo.account.status
# MAGIC     FROM   dbo.account
# MAGIC   -> tMap_1 (straight pass-through, no filter, no expression)
# MAGIC       AccountID   = row1.account_id      CustomerID = row1.customer_id
# MAGIC       AccountType = row1.account_type    Balance    = row1.balance
# MAGIC       DateOpened  = row1.date_opened     Status     = row1.status
# MAGIC   -> tMSSqlOutput (DWH.DimAccount, INSERT)
# MAGIC ```
# MAGIC
# MAGIC The Talend schema reads `date_opened` as `DATETIME2` with the display pattern
# MAGIC `dd-MM-yyyy`; the DWH column is `DATE`, so the value is truncated to a date here.
# MAGIC That pattern is also used when bronze landed `date_opened` as a string (CSV-style
# MAGIC ingestion) rather than a timestamp.
# MAGIC
# MAGIC Differences vs. Talend, by design:
# MAGIC * source is `dwh.bronze.account` (ticket 4) instead of a JDBC read of `dbo.account`;
# MAGIC * snake_case column names on the silver target;
# MAGIC * `MERGE INTO` on `account_id` instead of blind `INSERT`, so re-runs are idempotent;
...
CATALOG = "dwh"
BRONZE_TABLE = "dwh.bronze.account"
TARGET_TABLE = "dwh.silver.dim_account"
BUSINESS_KEY = "account_id"
DEFAULT_SOURCE_SYSTEM = "sample_db"
DATE_OPENED_PATTERN = "dd-MM-yyyy"

# Ticket-3 target schema for DimAccount:
#   AccountID   INT         -> account_id   INT            (PK / business key)
#   CustomerID  INT         -> customer_id  INT
#   AccountType VARCHAR(50) -> account_type STRING
#   Balance     MONEY       -> balance      DECIMAL(19,4)
#   DateOpened  DATE        -> date_opened  DATE
#   Status      VARCHAR(50) -> status       STRING
# `account_id` is nullable in the DataFrame schema (a cast never yields a non-nullable
...
def _date_opened(df: DataFrame):
    """`DATETIME2`/`DATE` casts directly; a string bronze value uses the Talend pattern."""
    if isinstance(df.schema["date_opened"].dataType, StringType):
        return F.coalesce(
            F.to_date(F.col("date_opened"), DATE_OPENED_PATTERN),
            F.to_date(F.col("date_opened")),
        ).alias("date_opened")
    return F.col("date_opened").cast(DateType()).alias("date_opened")


def transform_account(
    bronze_df: DataFrame, source_system: str = DEFAULT_SOURCE_SYSTEM
) -> DataFrame:
    """Explicit projection + casts from `dwh.bronze.account` to the silver schema."""
    ...
    return bronze_df.select(
        ...
        F.col("account_type").cast(StringType()).alias("account_type"),
        F.col("balance").cast(DecimalType(19, 4)).alias("balance"),
        _date_opened(bronze_df),
...
def deduplicate(df: DataFrame, key: str = BUSINESS_KEY) -> DataFrame:
    ...
    ordering = Window.partitionBy(F.col(key)).orderBy(
        ...
        *[F.col(c).asc_nulls_last() for c in TARGET_COLUMNS if c != key],
    )
    return (
...
def build(
    ...
    silver_df = deduplicate(transform_account(bronze_df, source_system)).cache()
...
if "dbutils" in globals():
    ...
    print(f"dim_account load complete: {rows} row(s)")
